 ...
 */
  ...
  * add load sensing for the GE Switch Module. The master resides in my GitHub repository.
  */
...
metadata {
	...
	tiles {
		...
		standardTile("indicator", "device.loadStatus", width: 1, height: 1, decoration: "flat") {
			state "when off", label:"", action:"loadWhenOn", icon:"st.indicators.lit-when-off"
			state "when on", label:"Powersense", action:"loadWhenOff", icon:"st.indicators.lit-when-on"
		}
...
def loadWhenOn() {
    ...
    log.debug "Load Sensing turned ON"
}
...
def loadWhenOff() {
    ...
    log.debug "Load Sensing turned OFF"
}
